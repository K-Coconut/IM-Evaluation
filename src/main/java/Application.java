...
//        List<Integer> budgets = Arrays.asList(properties.getProperty("budgets").split(" ")).stream().map(Integer::parseInt).collect(Collectors.toList());
        ...
        if (type.equals("gen")) {
            ...
            String mode = args[1];
            if (!mode.equals("gcomb") && !mode.equals("gcomb_epoch") && !mode.equals("imm") && !mode.equals("DDiscount")
                    && !mode.equals("SingleDiscount") && !mode.equals("interp_imm") && !mode.equals("gnn_greedy") && !mode.equals("imm_dense")) {
